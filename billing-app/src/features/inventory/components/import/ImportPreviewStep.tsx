...
import type { ImportConfirmResult, ImportRow } from '@/features/inventory/api';
import { ImportPreviewGrid } from '@/features/inventory/components/import/ImportPreviewGrid';
...
export function ImportPreviewStep({ importId, initialRows, initialCounts, onConfirmed }: ImportPreviewStepProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      const result = await inventoryApi.confirmImport(importId);
      ...
      setError(err instanceof ApiError ? err.message : 'Could not update inventory');
    } finally {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Every value below is editable. Fix errors, adjust quantities or prices, or skip a row — nothing is saved
          until you click Update Inventory.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <Button onClick={handleConfirm} disabled={isConfirming || updatableCount === 0}>
            {isConfirming ? 'Updating…' : `Update Inventory (${updatableCount} row${updatableCount === 1 ? '' : 's'})`}
          </Button>
          {errorCount > 0 && (
