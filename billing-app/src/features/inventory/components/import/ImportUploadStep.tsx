...
import type { ImportUploadResult } from '@/features/inventory/api';
...
export function ImportUploadStep({ onUploaded }: ImportUploadStepProps) {
  ...
  async function handleFileSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setIsUploading(true);
    try {
      const result = await inventoryApi.uploadImportFile(file);
      onUploaded(result);
    } catch (err) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          Supported formats: .xlsx, .csv. Files from other suppliers with differently named columns are fine — you'll
          get a chance to map them to the right fields next.
        </p>
      </CardContent>
    </Card>
  );
