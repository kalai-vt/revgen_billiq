...
import { ImportUploadStep } from '@/features/products/components/import/ImportUploadStep';
import { ImportMappingStep } from '@/features/products/components/import/ImportMappingStep';
import { ImportPreviewStep } from '@/features/products/components/import/ImportPreviewStep';
import { ImportSuccessReport } from '@/features/products/components/import/ImportSuccessReport';
import type { ImportConfirmResult, ImportRowListResult, ImportUploadResult } from '@/features/products/api';
...
export function ImportProductsPage() {
  ...
  async function handleUploaded(result: ImportUploadResult) {
    ...
    if (result.requires_mapping) {
      ...
      const rows = await productsApi.getImportRows(result.import_id);
      setRowsResult(rows);
      setStep('preview');
    }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link to="/products/import-history" className="text-sm text-primary hover:underline">
          View import history
        </Link>
      </div>
