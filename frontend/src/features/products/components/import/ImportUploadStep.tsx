...
import type { ImportUploadResult } from '@/features/products/api';
...
export function ImportUploadStep({ onUploaded }: ImportUploadStepProps) {
  ...
  async function handleFileSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setIsUploading(true);
    try {
      const result = await productsApi.uploadImportFile(file);
      onUploaded(result);
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>1. Upload Products File</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          Supported formats: .xlsx, .csv. Rows that match an existing product (by identifier) update it; new
          identifiers create a new product. Files with differently named columns are fine — you'll get a chance to
