...
import { CANONICAL_FIELDS, CANONICAL_FIELD_LABELS } from '@/features/customers/api';
import type { ColumnMapping, ImportRowListResult } from '@/features/customers/api';
...
export function ImportMappingStep({ importId, detectedHeaders, initialMapping, onMapped }: ImportMappingStepProps) {
  ...
  const canSubmit = Boolean(mapping.name) && (Boolean(mapping.mobile) || Boolean(mapping.email));

  async function handleSubmit() {
    if (!canSubmit) {
      setError('Map a Customer Name column and at least one of Mobile or Email before continuing.');
      return;
    }
    setError(null);
    setIsSubmitting(true);
    try {
      const result = await customersApi.confirmImportMapping(importId, mapping);
      onMapped(result);
    } catch (err) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">* Name is required, plus at least one of Mobile or Email.</p>
