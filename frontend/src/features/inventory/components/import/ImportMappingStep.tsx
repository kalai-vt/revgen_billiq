...
import { CANONICAL_FIELDS, CANONICAL_FIELD_LABELS } from '@/features/inventory/api';
import type { ColumnMapping, ImportRowListResult } from '@/features/inventory/api';
...
export function ImportMappingStep({ importId, detectedHeaders, initialMapping, onMapped }: ImportMappingStepProps) {
  ...
  const hasIdentifier = Boolean(mapping.pid || mapping.sku);
  const hasQuantity = Boolean(mapping.quantity);
  const canSubmit = hasIdentifier && hasQuantity;

  async function handleSubmit() {
    if (!canSubmit) {
      setError('Map at least a Product ID or SKU column, and a Quantity column, before continuing.');
      return;
    }
    setError(null);
    setIsSubmitting(true);
    try {
      const result = await inventoryApi.confirmImportMapping(importId, mapping);
      onMapped(result);
    } catch (err) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
          ...
          {CANONICAL_FIELDS.map((field) => (
            <div key={field} className="space-y-1.5">
              <Label>
                ...
                {(field === 'quantity' || field === 'pid') && <span className="text-destructive"> *</span>}
              </Label>
              <Select
        ...
        <p className="text-xs text-muted-foreground">* Required — map either Product ID or SKU, and Quantity.</p>
