...
import { useImportHistory } from '@/features/inventory/hooks/useImportHistory';
...
export function ImportHistoryPage() {
  ...
  async function handleDownloadErrorReport(importId: string) {
    try {
      const blob = await inventoryApi.downloadImportErrorReport(importId);
    ...
    <ModulePage
      header={
        <div>
          ...
          <p className="text-sm text-muted-foreground">Every bulk inventory import, with its outcome.</p>
        </div>
      }
      footer={
      ...
      <Table containerClassName="overflow-x-visible overflow-y-visible">
        ...
        <TableBody>
          ...
          {!isLoading && data?.items.length === 0 && (
            <TableRow>
              <TableCell colSpan={7}>
                <EmptyState icon={FileClock} title="No imports yet" description="Bulk inventory imports will show up here." />
              </TableCell>
            </TableRow>
          )}
