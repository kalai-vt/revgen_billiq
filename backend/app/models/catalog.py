...
class Product(Base):
    ...
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    cost_price: Mapped[float] = mapped_column(Float, default=0.0, index=True)
    selling_price: Mapped[float] = mapped_column(Float, default=0.0, index=True)
    tax_rate_percent: Mapped[float] = mapped_column(Float, default=0.0, index=True)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_now)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_now, onupdate=_now)
