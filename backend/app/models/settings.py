...
class Settings(Base):
    ...
    suspension_reason: Mapped[str | None] = mapped_column(String(50), nullable=True)
    reactivated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    reactivated_by: Mapped[str | None] = mapped_column(String(200), nullable=True)
    ...
    # NULL = never configured. Deliberately nullable rather than defaulted to a transport:
